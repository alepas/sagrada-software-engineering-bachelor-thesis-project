...
import it.polimi.ingsw.model.cards.concretePublicObjectiveCards.PublicObjectiveCard8;
...
public class Poc8Test {
    private PublicObjectiveCard8 card;
    private WPC wpc;
    ...
    public void before(){
        ...
        colDices.clear();
        colDices.add(dice0);    //red
        colDices.add(dice5);    //green
        colDices.add(dice10);   //violet
        colDices.add(dice15);   //yellow
        ...
        colDices.clear();
        colDices.add(dice2);    //yellow
        colDices.add(dice7);    //green
        colDices.add(dice12);
        colDices.add(dice16);   //green
        when(wpc.getColDices(2)).thenReturn(colDices);

        colDices.clear();
        colDices.add(dice3);     //green
        colDices.add(dice8);    //red
        colDices.add(dice13);   //yellow
        colDices.add(dice17);   //blue
        ...
        when(wpc.numDicesOfColor(Color.BLUE)).thenReturn(2);
        ...
        when(wpc.numDicesOfColor(Color.GREEN)).thenReturn(5);

        when(wpc.numDicesOfShade(1)).thenReturn(3);
        when(wpc.numDicesOfShade(2)).thenReturn(2);
        when(wpc.numDicesOfShade(3)).thenReturn(2);
        when(wpc.numDicesOfShade(4)).thenReturn(4);
        when(wpc.numDicesOfShade(5)).thenReturn(2);
        when(wpc.numDicesOfShade(6)).thenReturn(2);
    }

    @Test
    public void poc5Test(){
        Assert.assertEquals(2*POCConstants.POC8_SCORE, card.calculateScore(wpc));
    }
}
